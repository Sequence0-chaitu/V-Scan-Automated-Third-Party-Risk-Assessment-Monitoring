...
async def dashboard():
    # Serves the HTML page itself (not scan data), so it stays unprotected.
    # The page's own JS needs the API key to call /api/scan and /api/scans,
    # so it's injected into the HTML here rather than hardcoded client-side.
    if DASHBOARD_HTML_PATH.exists():
        html = DASHBOARD_HTML_PATH.read_text()
        html = html.replace("__VSCAN_API_KEY__", os.getenv("VSCAN_API_KEY", ""))
